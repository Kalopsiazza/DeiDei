...
test('preload only exposes named methods, strips IPC event and removes exact subscription',async()=>{
 ...
 assert.deepEqual(Object.keys(desktop.online).sort(),['openLobby','create','join','ready','start','setTurnLimit','changeRole','submit','returnLobby','leave','read','onChange'].sort());
