...
const {optionsFor,publicPlayers,remainingAt,canSelect,startReason,turnSummary,canSetTurnLimit,hostRecoveryText}=require('../build/online-model.cjs');
...
test('spectator, eliminated, forced, disconnected, revealing, pending and accepted never become editable',()=>{
 ...
 for(const modify of [v=>v.self.role='spectator',v=>v.members[0].participation='eliminated',v=>v.phase='revealing',v=>v.self.accepted_entry_id='Charge']){const copy=structuredClone(s);modify(copy.snapshot.view);assert.equal(canSelect(copy,12000),false);}
...
test('host configuration includes natural elimination and recovery copy never pauses the table',()=>{
 const state={status:'connected',snapshot:snapshot('selecting')};assert.equal(canSetTurnLimit(state),true);
 state.snapshot.view.members[0].participation='eliminated';assert.equal(canSetTurnLimit(state),true);
 state.snapshot.view.host_recovery={kind:'grace',remaining_ms:30000,deadline_at_ms:130000};assert.match(hostRecoveryText(state.snapshot,21000),/21 秒，其他人的牌局继续/);
 state.snapshot.view.pending_close={reason:'HOST_LEFT'};assert.equal(canSetTurnLimit(state),false);
 state.snapshot.view.pending_close=null;state.snapshot.view.host_id='p2';assert.equal(canSetTurnLimit(state),false);
});
