...
const policy={turn_ms:10000,early_reveal:true,spectator_cap:6,host_disconnect_grace_ms:30000,reveal_ms:1500,min_select_ms:300};
...
function snapshot(phase='lobby',seq='1',selfId='p1') {
 ...
 const view={source:'online',room_code:'ABCD2345',host_id:'p1',phase,has_password:false,policy:structuredClone(policy),policy_revision:'1',current_turn_ms:['selecting','revealing'].includes(phase)?10000:null,host_recovery:null,pending_close:null,members,match:phase==='lobby'?null:{match_id:state.match_id,mode_at_start:'multiplayer',turn_id:`${state.game_id}:t${state.turn_index}`,public_state:state,roster_profiles:members.map(({player_id,nickname,avatar_id,seat})=>({player_id,nickname,avatar_id,seat})),last_turn:phase==='revealing'?result:null,effective_outcome:null},self:{player_id:selfId,role:'player',seat:members.find(m=>m.player_id===selfId)?.seat??null,options:phase==='selecting'?structuredClone(samples.options):[],accepted_entry_id:null},timer:{kind:!['selecting','revealing'].includes(phase)?'none':phase==='revealing'?'reveal':'select',deadline_at_ms:!['selecting','revealing'].includes(phase)?null:phase==='revealing'?101500:110000,remaining_ms:!['selecting','revealing'].includes(phase)?null:phase==='revealing'?1500:10000},pause:null,close_reason:null};
...
class ScriptedSocket extends FakeSocket {
 ...
 respond(c){
  const ctl=this.controller;const roomId=c.op==='room.create'?(ctl.nextRoomId||'room1'):(ctl.view?.room_id||'room1');
  ...
  if(c.op==='room.create'||c.op==='room.join'){
   ...
   ctl.view=snapshot();ctl.view.room_id=roomId;if(c.op==='room.create')Object.assign(ctl.view.view.policy,c.payload.options);if(roomId==='room2')ctl.view.view.room_code='EFGH2345';if(c.op==='room.join'){ctl.view.view.host_id='p2';ctl.view.view.members=ctl.view.view.members.filter(p=>p.player_id!=='p1');ctl.view.view.members.push({player_id:'p1',nickname:'本机验收',avatar_id:'leaf',seat:null,role:'spectator',connected:true,ready:false,participation:'spectating',submission_state:'none',absence_count:0});ctl.view.view.self={player_id:'p1',role:'spectator',seat:null,options:[],accepted_entry_id:null};}
   this.ack(c,{room_id:roomId,room_code:ctl.view.view.room_code,...(c.op==='room.join'?{role:c.payload.role}:{})});
  }else if(c.op==='room.ready'){ctl.view.view.members.find(p=>p.player_id==='p1').ready=c.payload.ready;this.ack(c,{room_id:roomId,ready:c.payload.ready});}
   ...
   this.ack(c,{room_id:roomId,role:c.payload.role});
  }else if(c.op==='room.start'){const old=ctl.view.view;ctl.view=snapshot('selecting',String(BigInt(ctl.view.seq)+1n));ctl.view.room_id=roomId;Object.assign(ctl.view.view,{policy:old.policy,policy_revision:old.policy_revision,current_turn_ms:old.policy.turn_ms,timer:{kind:'select',remaining_ms:old.policy.turn_ms,deadline_at_ms:100000+old.policy.turn_ms}});this.ack(c,{room_id:roomId,match_id:'match1'});}
  else if(c.op==='room.set_turn_limit'){
   const v=ctl.view.view;
   if(v.host_id!=='p1'){this.fail(c,'NOT_HOST');return;}
   if(v.pending_close){this.fail(c,'ROOM_CLOSING');return;}
   if(c.payload.expected_policy_revision!==v.policy_revision){this.fail(c,'POLICY_STALE');return;}
   if(c.payload.turn_ms!==v.policy.turn_ms){v.policy.turn_ms=c.payload.turn_ms;v.policy_revision=String(BigInt(v.policy_revision)+1n);}
   this.ack(c,{room_id:ctl.view.room_id,turn_ms:v.policy.turn_ms,policy_revision:v.policy_revision,effective_from:'next_select'});
  }
  ...
  else if(c.op==='room.return_lobby'){const old=ctl.view.view;ctl.view=snapshot('lobby',String(BigInt(ctl.view.seq)+1n));ctl.view.room_id=roomId;Object.assign(ctl.view.view,{policy:old.policy,policy_revision:old.policy_revision});ctl.view.view.members.forEach(p=>p.ready=false);this.ack(c,{room_id:roomId});}
  else if(c.op==='room.leave'){ctl.view=null;this.ack(c,{room_id:roomId,left:true});return;}
  else this.ack(c,{room_id:roomId});
