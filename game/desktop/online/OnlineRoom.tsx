...
import { canSelect, describeError, optionsFor, publicPlayers, remainingAt, startReason, turnSummary } from './model';
import { groups, orderedOptions, shortcutEntry } from '../interaction';
import { ddText } from '../view-loop';
const api=window.desktop.online;
const initial:OnlineState={source:'online',status:'idle',hello:null,snapshot:null,pending:false,error:null,confirmed:null,remaining_ms:null,revision:0};
...
export function OnlineRoom({manual,onExit,Avatar,Modal}:Props) {
 ...
 const [turnMs,setTurnMs]=useState(12000),[early,setEarly]=useState(true),[cap,setCap]=useState(6);
 ...
 useEffect(()=>{
  const key=(e:KeyboardEvent)=>{
   if(e.repeat||e.isComposing||e.ctrlKey||e.metaKey||e.altKey||detail||resource||leaveConfirm||!editable||(e.target as HTMLElement).closest('input,textarea,select,dialog,button:not(.card-pick),[contenteditable="true"]'))return;
