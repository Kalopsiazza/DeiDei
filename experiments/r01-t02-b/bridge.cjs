...
class WorkerBridge {
  constructor(executable, args = [], timeout = 10000) {
    ...
    this.serial = 0; this.current = null; this.generations = new Set(); this.stopping = null;
  }
  start() {
    ...
    this.current = g; this.generations.add(g);
    ...
    child.stdout.on('data', chunk => {
      ...
      while ((end = g.buffer.indexOf(10)) >= 0) {
        ...
        try {
          ...
          if (r.v !== 1 || typeof r.ok !== 'boolean' || !g.pending.has(r.id) ||
